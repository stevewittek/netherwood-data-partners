...
const articleSitemap = await readFile(resolve(root, 'articles-sitemap.xml'), 'utf8');
assert.deepEqual([...articleSitemap.matchAll(/<loc>(.*?)<\/loc>/g)].map(m => m[1]).sort(), expected.filter(p => p.startsWith('/articles')).map(p => `${origin}${p}`).sort());
const titles = new Set(); const checks = [];
...
for (const route of expected) {
  ...
  if (article) {
    ...
    assert.equal(data.headline, article.title); assert.equal(data.dateModified, article.modifiedDate);
    assert.equal(data.mainEntityOfPage, `${origin}${route}`);
    for (const [, value] of article.html.matchAll(/(?:src|href)="([^"]+)"/g)) await checkUrl(value.replaceAll('&amp;', '&'));
    if (article.featuredImage) await checkUrl(article.featuredImage);
  }
  checks.push(route);
}
for (const file of ['404.html', 'admin/articles/index.html']) assert.match(await readFile(resolve(root, file), 'utf8'), /noindex/);
const robots = await readFile(resolve(root, 'robots.txt'), 'utf8');
assert.match(robots, /Disallow: \/admin\//); assert.ok(robots.includes(`${origin}/articles-sitemap.xml`));
assert.equal((await readFile(resolve(root, 'CNAME'), 'utf8')).trim(), 'netherwooddatapartners.com');
console.log(JSON.stringify({ event: 'static_release_checked', routes: checks, articleCount: snapshot.articleCount, contentDigest: snapshot.contentDigest }));
