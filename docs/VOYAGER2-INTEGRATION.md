...
## Public-site attribution capture: integrated
...
3. The integrated site uses `sessionStorage` only, preserving one first touch for the current browser tab. It uses no cookie or longer-lived local storage. Storage denial fails quietly. Any longer first-party persistence needs a separate owner-approved privacy/consent decision, bounded expiry and reset rule.
...
Implemented hidden metadata field names for the existing form:
...
These are untrusted metadata, not proof of permission or identity. They are added only when a valid campaign touch exists. The public browser does not send the marketing administrative token or conversion integration token. Existing Formspark validation, spam controls, native POST and email fallback remain intact.
