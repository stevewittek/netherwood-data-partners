...
10. **Voyager 2 handoff:** [VOYAGER2-INTEGRATION.md](VOYAGER2-INTEGRATION.md) specifies
    ...
    authentication, environment variables, event names and Ads readiness. The
    public website/form attribution fields are integrated on the release branch;
    reliable public token hosting remains future work.
...
14. **Integration validation passed:** 40 marketing tests, including a complete
    synthetic import → suppression → simulated send → tracked landing → inquiry →
    unsubscribe → suppressed re-import flow; strict marketing/backend/frontend
    TypeScript; repository-wide lint; both production builds; all 23 static route,
    metadata, link, schema and sitemap checks; 23 publication/admin and 3 pipeline
    tests; 70 backend tests; and the business-operations check. Browser acceptance
    covered 69 responsive renders, 22 form/readiness/attribution scenarios with 14
    locally intercepted submissions and 35 axe scans with zero reported violations.
    There were no unexpected external browser requests and no delivered mail.
    Windows-only build/route-check portability fixes remain included.
    [Original Voyager 1 evidence](evidence/2026-09-25/marketing/verification.md).
    Local runtime: Node 24.19.0 / pnpm 11.25.0; CI remains Node 22 / pnpm 11.19.0
    and includes the marketing safety tests. GitHub CI and deployment are the final
    release gates.
...
16. **Exact recommended next step:** the synthetic campaign trial is complete.
    Review and license one owner-approved Central New Jersey source list, retain
    its provenance and permitted-use terms, import it in preview mode, and review
    duplicates and suppression before any recipient list is frozen. Separately,
    choose a provider based on its written permission for that source and finish
    sender and reliable unsubscribe-hosting setup before production activation.
