...
function shutdown(signal: string): void { console.log(JSON.stringify({ event: "server_stopping", signal })); server.close((error) => { if (error) { console.error(JSON.stringify({ event: "server_stop_failed", message: error.message })); process.exitCode = 1; } }); }
