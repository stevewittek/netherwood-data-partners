...
function safePublicUrl(value: unknown, label: string): string | undefined {
  ...
  if (candidate.includes("\\")) throw new Error(`${label} contains a backslash`);
  if (candidate.startsWith("/") && !candidate.startsWith("//")) return candidate;
  let parsed: URL;
  try { parsed = new URL(candidate); } catch { throw new Error(`${label} must be a relative path or HTTPS URL`); }
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) throw new Error(`${label} must be a relative path or HTTPS URL`);
  return candidate;
}
...
function validateArticle(value: unknown, index: number, generatedAt: string): PublicArticleJson {
  ...
  const allowed = ["articleId", "title", "slug", "summary", "html", "plainText", "category", "tags", "author", "status", "isFeatured", "publishedDate", "createdDate", "modifiedDate", "featuredImage", "seoTitle", "seoDescription", "metaTitle", "metaDescription"];
  if (Object.keys(article).some(key => !allowed.includes(key))) throw new Error(`${prefix} has an unknown or private field`);
...
export function validateArticleSnapshot(value: unknown, options: SnapshotValidationOptions = {}): ArticleSnapshot {
  ...
  if (Object.keys(snapshot).some(key => !["format", "generatedAt", "articleCount", "contentDigest", "articles"].includes(key))) throw new Error("snapshot has an unknown field");
  if (!Array.isArray(snapshot.articles) || snapshot.articles.length > 10000) throw new Error("snapshot.articles must be an array of at most 10000 articles");
...
export async function readArticleSnapshot(path: string, options: SnapshotValidationOptions = {}): Promise<ArticleSnapshot> {
  ...
  if (details.size > 20_000_000) throw new Error("snapshot exceeds 20 MB limit");
